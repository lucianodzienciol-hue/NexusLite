...
// ===== LOAD =====
    ...
    function afterLoadFallback() {
      const headerImg = document.getElementById('headerBrandImg');
      const footerImg = document.getElementById('footerBrandImg');
      if (allConfig && allConfig.headerLogo) {
        const brandSrc = safeImg(allConfig.headerLogo);
        if (brandSrc) {
          if (headerImg) { headerImg.src = brandSrc; headerImg.style.display = ''; }
          if (footerImg) { footerImg.src = brandSrc; footerImg.style.display = ''; }
        }
      } else {
        if (headerImg) headerImg.style.display = 'none';
        if (footerImg) footerImg.style.display = 'none';
      }
      const companyName = (allConfig && allConfig.companyName) || 'Mi Tienda';
      const navBrand = document.getElementById('navbarBrandText');
      if (navBrand) navBrand.textContent = companyName;
      const footBrand = document.getElementById('footerBrandText');
      if (footBrand) footBrand.textContent = companyName;
      document.title = (allConfig && allConfig.siteTitle) || 'Mi Tienda Online';
    ...
    function buildOrderPdf(data) {
      ...
      const company = pdfSafe(allConfig.companyName || 'Mi Tienda');
